...
class LatentDiffusion(DDPM):
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    if ref_img_feature == None:
                        raise ValueError("Are you sure you do not have the reference image/feature as the condition?")
                    # print("in the get_input for getting learned condition.")
                    # print(ref_img_feature.size(), ref_img_feature.device)
                    # import sys;sys.exit(0)
                    c = self.get_learned_conditioning(xc, ref_img_feature)
                else:
                    c = self.get_learned_conditioning(xc.to(self.device))
                # print("the type of xc is: ", type(xc))
                # print(xc)
                # ['arafed * with a beard and a black shirt and a black jacket', 'a * with long hair']
                # import sys;sys.exit(0)
            else:
                c = xc
            if bs is not None:
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        # print(f"inside the apply_model...")
        # print(hasattr(self, "split_input_params"))  # False
        # import sys;sys.exit(0)
        if hasattr(self, "split_input_params"):
            ...
            ## this is the place which you applied the U-Net on the input.
            # print(self.model)   ## you will see the UNetModel here.
            # import sys;sys.exit(0)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        '''
            GX:
                log["samples"] and log["samples_scaled"] are output as 
                samples_gs-... samples_scaled_gs-...
                inpaint and plot_progressive_rows are both False in the default setup.
        '''
        if sample:
            # get denoise row
            with self.ema_scope("Plotting"):
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                ...
                print("doing the Quantized Denoised.")
